...
If ((Get-ChildItem *.pdb -Path .\lib -Recurse).Count -gt 0) {
	$packageTask = Create-Process .\NuGet.exe ("pack IkeCode.Data.Core.nuspec -Symbol -Verbosity Detailed")
...
Else {
	$packageTask = Create-Process .\NuGet.exe ("pack IkeCode.Data.Core.nuspec -Verbosity Detailed")
	$packageTask.Start() | Out-Null
	$packageTask.WaitForExit()
